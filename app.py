...
MAX_FILES_PER_SESSION = 4  # 3 compulsory + 1 optional
...
class DatabaseManager:
    ...
    def create_invitation(self, manager_id: int) -> str:
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO user_invitations (invitation_code, manager_id, expires_at)
...
class FileValidator:
    ...
    def get_file_type(filename: str) -> str:
        ...
        if ext in REQUIRED_FILE_TYPES['spreadsheet']:
            return 'spreadsheet'
        elif ext in REQUIRED_FILE_TYPES['document']:
            return 'document'
        elif ext in REQUIRED_FILE_TYPES['image']:  # Fixed typo here
            return 'image'
        return 'other'

    @staticmethod
    def validate_required_files(uploaded_files: List[str]) -> tuple[bool, str]:
        file_types = {FileValidator.get_file_type(f) for f in uploaded_files}
        missing_types = []
        
        if 'spreadsheet' not in file_types:
            missing_types.append("Spreadsheet (CSV/XLSX)")
        if 'document' not in file_types:
            missing_types.append("PDF document")
        if 'image' not in file_types:
            missing_types.append("Image file")
            
        if missing_types:
            return False, f"❌ Missing required file types: {', '.join(missing_types)}"
        return True, ""
    ...
    def validate_extension(filename: str) -> tuple[bool, str]:
        ...
        allowed_exts = ", ".join(FileValidator.ALLOWED_EXTENSIONS)
        if ext not in FileValidator.ALLOWED_EXTENSIONS:
            return False, f"❌ File extension {ext} not allowed. Only {allowed_exts} are permitted."
        return True, ""
...
Supported formats: PDF, DOCX, XLSX, ZIP, Images (JPG, PNG, GIF)
Max files per session: 4 (3 compulsory + 1 optional)

File naming convention:
ProjectCode_DocumentType_Date.extension
Example: ABC123_Report_20240315.pdf
        """
...
Extensions: Only PDF, DOCX, XLSX, ZIP, Images (JPG, PNG, GIF) allowed
File naming: ProjectCode_DocumentType_Date.extension
Content validation:
   • PDFs: Must have at least 1 page
   • Excel: Must have sheets with headers
   • Word: Must contain at least 100 characters
   • Images: Automatically processed
Security: Files scanned for threats
Structure: Proper folder organization required
...
/help - Show this help
    ...
    async def upload_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            f"You can upload up to {MAX_FILES_PER_SESSION} files\n\n"
    ...
    async def handle_document(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if session['files_uploaded'] == 3 and len(update.message.document) > 1:
            await update.message.reply_text(f"You can only upload 1 more file ({session['files_uploaded']}/{MAX_FILES_PER_SESSION} used).")
            return
    ...
    async def delayed_upload_notification(self, user_id: int, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            valid, error = FileValidator.validate_required_files(session['uploaded_files'])
...
Use /cancel to cancel this session.
    ...
    async def handle_invite_user(self, query: CallbackQuery, context: ContextTypes.DEFAULT_TYPE):
        ...
        with self.db.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO user_invitations (manager_id, invitation_code, expires_at)
                    VALUES (%s, %s, %s)
                """, (manager_id, invitation_code, expires_at))
                conn.commit()
        ...
        await query.edit_message_text(
            f"🔗 Here's your invitation link (valid for 24 hours):\n\n"
            f"{invitation_link}\n\n"
            "Share this link with the user you want to invite.\n"
            "When they click it, they'll be able to request access to the bot."
        )
